...
public abstract class MqttWireMessage {	
	...
	public static final byte MESSAGE_TYPE_PUBMISS = 15;
	...
	private static final String PACKET_NAMES[] = { "reserved", "CONNECT", "CONNACK", "PUBLISH",
			...
			"UNSUBSCRIBE", "UNSUBACK", "PINGREQ", "PINGRESP", "DISCONNECT", "PUBMISS" };
	...
	public String getKey() {
		...
		return new Integer(Math.abs(getMessageId())).toString();
	}
	...
	private static MqttWireMessage createWireMessage(InputStream inputStream) throws MqttException {
		try {
			...
			else if (type == MqttWireMessage.MESSAGE_TYPE_PUBMISS) {
				result = new MqttPubMiss(info, data);
			}
			else {
