...
public class RocketmqResource extends PluginResource {
	...
	public MQPushConsumer usePushConsumer(String consumerGroup, String topic, String tag, MessageListener messageListener) {
		return usePushConsumer(consumerGroup, topic, tag, messageListener, ConsumeFromWhere.CONSUME_FROM_FIRST_OFFSET, 15,
				MessageModel.CLUSTERING);
	}

	public MQPushConsumer usePushConsumer(String consumerGroup, String topic, String tag, MessageListener messageListener, 
			ConsumeFromWhere fromWhere, int batchMaxSize, MessageModel model) {
		try {
	...
	public SendResult send(String producerGroup, String topic, String tags, String keys, byte[] data) {
		return send(useProducer(producerGroup), new Message(topic, tags, keys, data));
	}
	
	public SendResult sendTimeout(String producerGroup, String topic, String tags, String keys, byte[] data, int timeout) {
		return sendTimeout(useProducer(producerGroup), new Message(topic, tags, keys, data), timeout);
	}
	
	public void sendOneway(String producerGroup, String topic, String tags, String keys, byte[] data) {
		sendOneway(useProducer(producerGroup), new Message(topic, tags, keys, data));
	}
